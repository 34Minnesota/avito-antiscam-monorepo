...
func loadVersions(ctx context.Context, tx *sql.Tx, userID domain.UserID) ([]domain.ScenarioProgress, scenarioIndexes, error) {
	rows, err := tx.QueryContext(ctx, versionsQuery, userID.UUID())
	if err != nil {
		return nil, scenarioIndexes{}, dependencyError(err)
	}
	defer closeRows(rows)
	indexes := scenarioIndexes{byID: make(map[uuid.UUID]int), versions: make(map[uuid.UUID]domain.Version)}
...
func scanVersionRow(rows *sql.Rows, scenarioID *uuid.UUID, slug, title, role *string,
...
func loadRecentAttempts(ctx context.Context, tx *sql.Tx, userID domain.UserID, limit int, scenarios []domain.ScenarioProgress, indexes scenarioIndexes) error {
	rows, err := tx.QueryContext(ctx, recentAttemptsQuery, userID.UUID(), limit)
	if err != nil {
...
func loadOutdatedAttempts(ctx context.Context, tx *sql.Tx, userID domain.UserID) ([]domain.OutdatedActiveAttempt, error) {
	rows, err := tx.QueryContext(ctx, outdatedAttemptsQuery, userID.UUID())
	if err != nil {
...
func closeRows(rows *sql.Rows) {
	if err := rows.Close(); err != nil {
		slog.Error("close progress query rows", "error", err)
	}
}

func rollback(tx *sql.Tx) {
	if err := tx.Rollback(); err != nil && !errors.Is(err, sql.ErrTxDone) {
		slog.Error("rollback progress read transaction", "error", err)
	}
}
